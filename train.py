...
EPISODE_DIR = (
    "F:/rl/finalSubmission/Agent_share/full_episodes/top_agents"
)
MODEL_DIR = "./"
...
def find_json_files(base_dir):
    ...
    json_files = []
    first_level_dirs = [x for x in base_dir.iterdir() if x.is_dir()]
    for dir1 in first_level_dirs:
        second_level_dirs = [x for x in dir1.iterdir() if x.is_dir()]
        for dir2 in second_level_dirs:
            # Collect all JSON files in these directories
            for json_file in dir2.glob('*.json'):
...
def create_dataset_from_json(episode_dir, team_name="Toad Brigade"):
    ...
    episodes = find_json_files(episode_dir)
    # clean episode files
    episodes = [
        ...
        for i in range(len(json_load["steps"]) - 1):
            if json_load["steps"][i][index]["status"] == "ACTIVE":
                ...
                for action in actions:
                    _, label, unit_pos = to_label(action, obs)
